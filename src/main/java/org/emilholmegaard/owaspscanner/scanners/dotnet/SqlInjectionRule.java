...
public class SqlInjectionRule extends AbstractDotNetSecurityRule {
    ...
    private static final String REMEDIATION = "Use parameterized queries, ORMs, or stored procedures instead of string concatenation. "
            +
            "For Entity Framework, use LINQ queries. With ADO.NET, always use SqlParameter objects.";
    private static final String REFERENCE = "https://cheatsheetseries.owasp.org/cheatsheets/DotNet_Security_Cheat_Sheet.html#sql-injection";

    // Optimized SQL injection detection pattern with bounded quantifiers to prevent
    ...
    protected boolean checkViolation(String line, int lineNumber, RuleContext context) {
        ...
        if (SQL_INJECTION_PATTERN.matcher(line).find()) {
            ...
            boolean isLikeQueryWithConcatenation = line.contains("LIKE") &&
                    line.contains("'%") &&
                    line.contains("+") &&
                    line.contains("%'");
